...
# Solo las clases faltantes (las de bajo pick-rate)
RARE_CLASSES = ["Anchor", "Bruiser", "Center", "Flanker", "Forward", "Grinder", "Sprinter", "Support"]
...
def analyze_rare_classes():
    ...
    # Filtro inmediato: descartar Strikers y Defenders para acelerar
    df = df[df["champ_class"].isin(RARE_CLASSES)].copy()
    ...
    print("[INFO] Computando historiales de Clases Raras en bloques de 10...")
    ...
    if df_eval.empty:
        print("[ERROR] No hay datos de esas clases para agrupar.")
        return
    ...
    md_output = []
    md_output.append("# 🛡️ Reporte Secundario: Clases Raras / Bajo Pick-Rate\n")
    md_output.append("> **Filtros Aplicados:** Bloques de 10. *Filtro de volumen reducido a mínimo 2 participaciones por comp.* ¡Atención a la varianza por baja data!\n")
    md_output.append("| Clase | Formación | Win Cond Dominante | Bloques Analizados | Top Scheme Recomendado |\n")
    md_output.append("| :--- | :--- | :--- | :--- | :--- |\n")
    ...
    for (cls, comp), data in comp_groups:
        if len(data) < 2:  # REDUCIDO DRASTICAMENTE PARA INCLUIRLAS TODAS
            continue
        ...
        for scheme in scheme_cols:
            comp_stats.append({
                ...
                "reliability": (data[scheme] >= 1000).mean() * 100
            })
        ...
        section = [
            ...
            f"- **Bloques Analizados:** {len(data)} (DATA MUY BAJA, RIESGO DE RNG)",
        ...
        for st in comp_stats[:3]:
            ...
            safe_std = 0 if pd.isna(st['std_dev']) else st['std_dev']
            section.append(f"| {st['scheme']} | {st['mean_ev']:+5.0f} | ±{safe_std:.0f} | {st['reliability']:.1f}% | {tier} |")
        
        detailed_sections.append("\n".join(section) + "\n")

    full_md = "".join(md_output) + "\n---\n\n## 🔍 Análisis Profundo de Clases Raras\n\n" + "\n".join(detailed_sections)
    
    report_path = DATA_DIR / "scheme_tier_list_rare_classes.md"
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(full_md)
        
    print(f"\n[OK] Reporte RARE CLASSES generado en: {report_path}")
    print(f"[OK] Analizadas {valid_comps} Composiciones secundarias.")
