...
def calculate_points(is_win, elims, deposits, wart_dist):
    pts = 300 if is_win else 0
    pts += int(elims) * 80
    pts += int(deposits) * 50
    pts += floor(float(wart_dist) / 80) * 45
    return pts
...
def identify_roles_and_features(row):
    ...
    feat = {
        "moki_token_id": str(moki_tid),
        "enemy_champ_token_id": str(enemy_tid),
        "moki_vs_enemy": f"{moki_tid}_vs_{enemy_tid}",
...
def main():
    ...
    for moki_id in moki_ids:
        ...
        results.append({
            ...
            "pred_deposits": sum(pred_deposits),
