...
=============================================
Entrena modelos utilizando variables de identidad, composición e interacciones.
Implementa "Time Weighting", carga hiperparámetros optimizados y utiliza
arquitectura de Cascada (Stacking) para Score y WinRate.
"""
...
def calculate_time_weights(dates_series):
    try:
        ...
        for d in dates:
            diff = (today - d).days
            if diff <= 7: weights.append(2.0)
            elif diff <= 30: weights.append(1.5)
...
def train_models():
    ...
    # 1. ENTRENAR MODELOS AUXILIARES (Base para el Stacking)
    print("\n" + "="*50)
    print("FASE 1: ENTRENAMIENTO DE MODELOS AUXILIARES")
    print("="*50)

    # Definir modelos auxiliares
    aux_configs = [
    ...
    for cfg in aux_configs:
        ...
        y_aux = df[cfg['target']].fillna(0).astype(float if cfg['class'] == CatBoostRegressor else int)
        
        X_t, X_v, y_t, y_v, w_t, _ = train_test_split(X_base, y_aux, sample_weights, test_size=0.2, random_state=42)
        
        model = cfg['class'](iterations=500, learning_rate=0.05, depth=6, eval_metric=cfg['metric'], random_seed=42, verbose=False)
        model.fit(X_t, y_t, cat_features=cat_features, sample_weight=w_t, eval_set=(X_v, y_v), use_best_model=True)
        model.save_model(str(MODELS_DIR / cfg['file']))
        
        # Generar predicciones para TODO el dataset para usarlas como features
    ...
    # 2. ENTRENAR MODELOS PRINCIPALES (Con Features de Cascada)
    print("\n" + "="*50)
    print("FASE 2: ENTRENAMIENTO DE MODELOS PRINCIPALES (STACKING)")
    print("="*50)

    # Nuevas features incluyen las predicciones de la Fase 1
    cascade_features = [f"pred_{cfg['name'].lower()}" for cfg in aux_configs]
    X_cascade = pd.concat([X_base, df[cascade_features]], axis=1)
    
    # IMPORTANTE: Los modelos principales deben saber que las nuevas features son NUMÉRICAS
    # CatBoost lo detecta automáticamente si no están en cat_features.
    ...
    def fit_main(model_class, target_col, name, filename, params_name):
        ...
        X_t, X_v, y_t, y_v, w_t, _ = train_test_split(X_cascade, y, sample_weights, test_size=0.2, random_state=42)
    ...
    # Win Condition (Multiclase) - También puede beneficiarse de las cascade features
    ...
    model_cond = CatBoostClassifier(iterations=500, learning_rate=0.05, depth=6, loss_function='MultiClass', random_seed=42, verbose=100)
    model_cond.fit(X_t, y_t, cat_features=cat_features, sample_weight=w_t, eval_set=(X_v, y_v), use_best_model=True)
