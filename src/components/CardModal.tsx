...
export default function CardModal({
  ...
  const isCharacterImage =
    card.cardType === 'MOKI' &&
    !!(card.custom.characterImage || card.custom.imageUrl);
            ...
            {card.cardType === 'SCHEME' ? (
              ...
              <div className={styles.modalBody}>
                <div className={styles.imageSection}>
                  <div
                    className={`${styles.cardImageWrapper} ${isCharacterImage ? styles.characterImage : ''}`}
                  >
                    <NextImage
                      src={
                        card.custom.characterImage ||
                        card.custom.imageUrl ||
                        card.image
                      }
                      alt={card.name}
                      fill
                      ...
                      style={{
                        objectFit: isCharacterImage ? 'contain' : 'cover',
                      }}
                      priority
                    />
                  </div>
                ...
                <div className={styles.infoSection}>
                  ...
                  <div className={styles.detailsList}>
                    ...
                    <div className={styles.detailRow}>
                      ...
                      <span className={styles.statValue}>
                        {card.custom.stars > 0 ? `${card.custom.stars} ★` : '-'}
                      </span>
                    </div>
                  </div>
                    ...
                    <>
                      <div className={styles.performanceGrid}>
                        <div className={styles.perfHeader}>
                          {matchLimit === 'ALL'
                            ? 'PERFORMANCE'
                            : `LAST ${matchLimit} MATCHES AVERAGE`}
                        </div>
                        <div className={styles.perfItem}>
                          <span className={styles.perfLabel}>ELIMS</span>
                          <span className={styles.perfValue}>
                            {getStatValueByLimit(card, 'eliminations', matchLimit).toFixed(1)}
                          </span>
                        </div>
                        <div className={styles.perfItem}>
                          <span className={styles.perfLabel}>WART</span>
                          <span className={styles.perfValue}>
                            {getStatValueByLimit(card, 'wartDistance', matchLimit).toFixed(1)}
                          </span>
                        </div>
                        <div className={styles.perfItem}>
                          <span className={styles.perfLabel}>BALLS</span>
                          <span className={styles.perfValue}>
                            {getStatValueByLimit(card, 'deposits', matchLimit).toFixed(1)}
                          </span>
                        </div>
                        <div className={styles.perfItem}>
                          <span className={styles.perfLabel}>SCORE</span>
                          <span className={styles.perfValue}>
                            {getStatValueByLimit(card, 'score', matchLimit).toFixed(1)}
                          </span>
                        </div>
                        <div className={styles.perfItem}>
                          ...
                          <span className={styles.perfValue}>
                            {(() => {
                              const wr = getStatValueByLimit(card, 'winRate', matchLimit);
                  ...
                  {card.cardType === 'MOKI' && (
                    <div className={styles.statsGrid}>
                      ...
                      <div className={styles.statItem}>
                        ...
                        <span className={styles.statValue}>
                          {card.custom.strength?.toFixed(1) || '0.0'}
                        </span>
                      </div>
                      <div className={styles.statItem}>
                        <span className={styles.statLabel}>SPD</span>
                        <span className={styles.statValue}>
                          {card.custom.speed?.toFixed(1) || '0.0'}
                        </span>
                      </div>
                      <div className={styles.statItem}>
                        <span className={styles.statLabel}>DEF</span>
                        <span className={styles.statValue}>
                          {card.custom.defense?.toFixed(1) || '0.0'}
                        </span>
                      </div>
                      <div className={styles.statItem}>
                        <span className={styles.statLabel}>DEX</span>
                        <span className={styles.statValue}>
                          {card.custom.dexterity?.toFixed(1) || '0.0'}
                        </span>
                      </div>
                      <div className={styles.statItem}>
                        <span className={styles.statLabel}>FOR</span>
                        <span className={styles.statValue}>
                          {card.custom.fortitude?.toFixed(1) || '0.0'}
                        </span>
                      </div>
                      <div
                        className={`${styles.statItem} ${styles.mainStatItem}`}
                      >
                        <span className={styles.statLabel}>TOTAL</span>
                        <span className={styles.totalValue}>
                          {card.custom.totalStats?.toFixed(1) || '0.0'}
                        </span>
                      </div>
                      <div
                        className={`${styles.statItem} ${styles.mainStatItem}`}
                      >
                        ...
                        <span className={styles.totalValue}>
                          {card.custom.train?.toFixed(1) || '0.0'}
                        </span>
                      </div>
                    </div>
                  )}
