...
export default function MatchHistoryModal({
  ...
  matches.forEach((m) => {
    ...
    totalScore +=
      ...
      (m.deposits || 0) * 50 +
      (m.eliminations || 0) * 80 +
      Math.floor((m.wart_distance || 0) / 80) * 45;
  });
  ...
  const avgDeposits =
    numMatches > 0 ? (totalDeposits / numMatches).toFixed(1) : '0';
  const avgWart = numMatches > 0 ? Math.round(totalWart / numMatches) : 0;

  // Resolve market link
  const mokiMetadata = mokiMetadataRaw as Record<string, any>;
  const activeMetadata = Object.values(mokiMetadata).find(
    (m) => String(m.id) === String(tokenId)
  );
  ...
  const renderMatchDetailsModal = () => {
    ...
    const teammates = players.filter(
      (p: any) => p.team === m.moki_team && p.mokiId !== m.moki_id
    );
    ...
    const getPlayerPerformance = (mokiId: string, team: number) => {
      let elims = 0,
        deps = 0,
        wart = 0;
      const playerResult = m.match_data?.result?.players?.find(
        (pr: any) => pr.mokiId === mokiId
      );
      if (playerResult) {
      ...
      return (
        <div key={`${type}-${p.mokiId}`} className={styles.mokiDetailCard}>
          <div className={styles.mokiCardHeader}>
            <span className={styles.mokiName} title={p.name}>
              {p.name}
            </span>
          </div>
          ...
          <div className={styles.body}>
            {loading ? (
              ...
              <div className={styles.matchesList}>
                {matches.map((m) => {
                  const players = m.match_data?.players || [];
                  const targetMokiList = players.filter(
                    (p: any) => p.mokiId === m.moki_id
                  );
                  const teammates = players.filter(
                    (p: any) => p.team === m.moki_team && p.mokiId !== m.moki_id
                  );
                  const opponents = players.filter(
                    (p: any) => p.team !== m.moki_team
                  );
                  const isWinner = m.team_won === m.moki_team;
                  ...
                  return (
                    <div
                      key={m.match_id}
                      className={`${styles.matchCard} ${isWinner ? styles.winCard : styles.lossCard}`}
                    >
                      <div className={styles.matchMeta}>
                        <div
                          className={`${styles.resultBadge} ${isWinner ? styles.winText : styles.lossText}`}
                        >
                        ...
                        <div className={styles.metaRow}>
                          <span className={styles.metaLabel}>Win Type:</span>{' '}
                          <span className={styles.capitalize}>
                            {m.win_type.toLowerCase() === 'eliminations'
                              ? 'combat'
                              : m.win_type.toLowerCase()}
                          </span>
                        </div>
                      </div>
                        ...
                        <div className={styles.scoreBox}>
                          ...
                          <span className={styles.scoreValue}>
                            {(isWinner ? 300 : 0) +
                              m.deposits * 50 +
                              m.eliminations * 80 +
                              Math.floor(m.wart_distance / 80) * 45}
                          </span>
                        </div>
                      </div>
                          ...
                          <div className={styles.teamContainer}>
                            ...
                            <div className={styles.playersRow}>
                              {targetMokiList.map((p: any, idx: number) => (
                                <div
                                  key={`self-${idx}`}
                                  className={`${styles.playerAvatar} ${styles.currentPlayer}`}
                                  title={`${p.name} (You)`}
                                >
                                  {p.imageUrl && (
                                    <NextImage
                              ...
                              {teammates.length === 0 &&
                                targetMokiList.length === 0 && (
                                  <span className={styles.noData}>Unknown</span>
                                )}
                            </div>
                          </div>
                          ...
                          <div className={styles.teamContainer}>
                            ...
                            <div className={styles.playersRow}>
                              {opponents.map((p: any, idx: number) => (
                                <div
                                  key={`opp-${idx}`}
                                  className={`${styles.playerAvatar} ${idx === 0 ? styles.leadOpponent : ''}`}
                                  title={p.name}
                                >
                                  {p.imageUrl && (
                                    <NextImage
