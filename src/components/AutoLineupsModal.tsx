...
export default function AutoLineupsModal({
  ...
  const handleDelete = (id: string) => {
    ...
    if (lineups.length === 1) {
      // If last one was deleted, close modal
  ...
  const commitEdit = () => {
    ...
    if (editingId && trimmed.length > 0) {
      setLineups(
        lineups.map((lineup) => (lineup.id === editingId ? { ...lineup, name: trimmed } : lineup))
      );
    }
    setEditingId(null);
  };
                    ...
                    <div className={styles.lineupHeader}>
                      {editingId === lineup.id ? (
                        ...
                        <h3 className={styles.lineupName}>
                          <span className={styles.nameText} title={lineup.name}>
                            {lineup.name}
                          </span>
                          <button
