...
export default function ContestTypeModal({ isOpen, onClose, onGenerate }: ContestTypeModalProps) {
    ...
    const totalCount = counts.legendary + counts.epic + counts.rare + counts.basic;
                ...
                <div className={styles.content}>
                    ...
                    {selectedType === 'OTHER' && (
                        <div className={styles.customCountsContainer}>
                            <h3 className={styles.customTitle}>Select Rarities (Total: {totalCount}/4)</h3>
                            <div className={styles.counters}>
                                {(Object.keys(counts) as Array<keyof ExactCounts>).map(rarity => (
                                    <div key={rarity} className={styles.counterRow}>
                                        ...
                                        <div className={styles.counterControls}>
                                            ...
                                            <span className={styles.countValue}>{counts[rarity]}</span>
                                            <button
                                                className={styles.counterBtn}
                            ...
                            {totalCount !== 4 && (
                                <div className={styles.errorMsg}>You must select exactly 4 cards.</div>
                            )}
                        </div>
                    )}
                </div>
