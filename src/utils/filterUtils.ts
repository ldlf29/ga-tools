...
import { matchesSpecialization } from './specializationUtils';
...
export const matchesFilter = (
  ...
  if (filters.fur.length > 0) {
    if (!card.custom.fur || !filters.fur.includes(card.custom.fur))
      return false;
  }
  // STARS
  ...
  if (filters.customClass.length > 0) {
    if (!card.custom.class || !filters.customClass.includes(card.custom.class))
      return false;
  }
  // SPECIALIZATION (Sorting + Threshold Filtering)
  if (filters.specialization.length > 0) {
    for (const spec of filters.specialization) {
      if (!matchesSpecialization(card, spec, filters.matchLimit)) {
        return false;
      }
    }
  }
