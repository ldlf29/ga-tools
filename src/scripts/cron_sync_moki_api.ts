...
async function run() {
    ...
    for (let i = 0; i < allIds.length; i += BATCH_SIZE) {
        ...
        if (!response.ok) {
            console.error(`[Cron Sync Moki] Game API error on batch ${i}: ${response.status}`);
            continue;
        }
    ...
    console.log(`[Cron Sync Moki] Received ${apiResults.length} records from API.`);
    ...
    if (classChanges.length > 0) {
        console.log(`[Cron Sync Moki] Logging ${classChanges.length} class changes...`);
        const { error: logErr } = await supabaseAdmin
            .from('class_changes')
            .insert(classChanges);

        if (logErr) console.error('[Cron Sync Moki] Error logging class changes:', logErr);
        else {
            try {
                const { DiscordService } = await import('../services/DiscordService');
                await DiscordService.notifyClassChanges(classChanges);
            } catch (discordErr) {
                console.error('[Cron Sync Moki] Discord notification failed:', discordErr);
            }
        }
    }
    ...
    if (updatesToSupabase.length > 0) {
        console.log(`[Cron Sync Moki] Upserting ${updatesToSupabase.length} Moki stats...`);
    ...
    console.log(`[Cron Sync Moki] Sync Complete. Duration: ${Date.now() - startTime}ms`);
