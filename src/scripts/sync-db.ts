...
const STATS_URL =
  'https://docs.google.com/spreadsheets/d/e/2PACX-1vQAcXVDO4ylx4jU6KEjceneqnNYRyL6MB3R0myZE5bF1_Th8q4F79eUZsPZ-93pojf6UxUE1OiAGZEC/pub?output=csv';
...
async function main() {
  console.log(
    '🚀 [Sync-DB] Sincronización con Google Sheets DESACTIVADA por el usuario (script conservado para backup).'
  );
  return;
  ...
  try {
    ...
    if (fetchError) {
      console.error(
        '⚠️ Could not fetch existing stats for changelog:',
        fetchError?.message
      );
    }
    ...
    for (const stat of statsRecords) {
      ...
      if (existingClass && newClass && existingClass !== newClass) {
        console.log(
          `📝 Class change detected: ${stat.name} (${existingClass} → ${newClass})`
        );
        classChanges.push({
          moki_name: stat.name,
    ...
    if (classChanges.length > 0) {
      ...
      if (newChanges.length > 0) {
        ...
        if (changelogError) {
          console.error(
            '⚠️ Failed to log class changes:',
            changelogError?.message
          );
        } else {
          console.log(
            `✅ Logged ${newChanges.length} class change(s) to changelog.`
          );
    ...
    for (let i = 0; i < statsRecords.length; i += CHUNK_SIZE) {
      ...
      if (error) {
        console.error(
          `❌ Error upserting chunk ${i}-${i + CHUNK_SIZE}:`,
          error
        );
        throw error;
      }
...
function parseStatsCSV(csvText: string): MokiStats[] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    statsRecords.push({
      ...
      total_stats: parseNum(
        row['total stats'] || row['totalstats'] || row['total']
      ),
