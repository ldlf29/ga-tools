...
async function run() {
  ...
  for (let i = 0; i < allTokenIds.length; i += CONCURRENCY_LIMIT) {
    ...
    console.log(
      `[Cron Train] Processing Mokis ${i} to ${i + chunkIds.length}...`
    );
    ...
    const fetchPromises = chunkIds.map(async (tokenId) => {
      try {
        ...
        if (!response.ok) {
          console.error(
            `[Cron Train] API Error for tokenId ${tokenId}: ${response.status}`
          );
          return null;
        }
        ...
        const stats =
          data.performance && data.performance.stats
  ...
  console.log(
    `[Cron Train] Fetched ${results.length} records. Syncing to DB...`
  );
  ...
  if (upcomingMatchesInserts.length > 0) {
    ...
    for (let i = 0; i < uniqueMatchesInfo.length; i += DB_CHUNK_SIZE) {
      ...
      if (error) {
        console.error(
          '[Cron Train] Warning: Error upserting upcoming matches:',
          error
        );
      } else {
  ...
  console.log(
    `[Cron Train] Sync Complete. Updated ${successCount} Mokis & ${matchesCount} Matches.`
  );
