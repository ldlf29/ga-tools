...
async function run() {
    ...
    if (matchHistoryRecords.length > 0) {
        console.log(`[Cron Matches] Upserting ${matchHistoryRecords.length} records to DB...`);
    ...
    console.log(`[Cron Matches] Sync Complete. Upserted ${recordsUpserted}. Duration: ${Date.now() - startTime}ms`);
