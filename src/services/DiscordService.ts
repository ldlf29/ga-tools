export class DiscordService {
   ...
   */
  static async notifyClassChanges(
    changes: { moki_name: string; old_class: string; new_class: string }[]
  ) {
    if (!this.DISCORD_TOKEN || !this.CHANNEL_ID) {
      console.warn(
        '⚠️ Discord credentials missing. Skipping Discord notification.'
      );
      return;
    }
    ...
    try {
      ...
      const descriptionLines = changes.map(
        (c) =>
      ...
      const response = await fetch(
        `${this.BASE_URL}/channels/${this.CHANNEL_ID}/messages`,
        {
          method: 'POST',
          headers: {
      ...
      if (!response.ok) {
        ...
        console.log(
          `✅ Sent Discord notification for ${changes.length} class changes.`
        );
      }
    } catch (error) {
