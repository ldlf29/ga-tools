...
export class AlchemyService {
    ...
    private constructor() {
        ...
        if (!this.apiKey) {
            console.warn('[AlchemyService] ALCHEMY_API_KEY is not defined in environment variables.');
        }
     ...
     */
    public async getWalletNFTs(walletAddress: string, contractAddress: string): Promise<AlchemyNFT[]> {
        ...
        console.log(`[AlchemyService] Fetching NFTs for wallet ${walletAddress} on contract ${contractAddress}`);
        ...
        try {
            ...
            console.log(`[AlchemyService] Successfully fetched ${allNfts.length} NFTs.`);
            return allNfts;
    ...
    public parseAlchemyNFTs(nfts: AlchemyNFT[]): EnhancedCard[] {
        return nfts.map(nft => {
            ...
            const getAttr = (name: string) =>
                attributes.find(a => a.trait_type?.toLowerCase() === name.toLowerCase())?.value?.toString();
            ...
            if (maybeTokenId === undefined) {
                console.error(`[AlchemyService] MISSING tokenId on NFT item:`, JSON.stringify(nft).substring(0, 200));
            } else if (typeof maybeTokenId === 'string' && maybeTokenId.startsWith('0x')) {
