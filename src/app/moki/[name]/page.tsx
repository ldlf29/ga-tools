...
export default async function MokiPage({ params }: { params: Promise<{ name: string }> }) {
    const p = await params;
    const decodedName = decodeURIComponent(p.name);
    ...
    if (!mokiKey) {
        return (
            <div style={{ padding: '40px', textAlign: 'center', color: 'white', fontFamily: 'var(--font-geist-sans)' }}>
                <h1>Moki Not Found</h1>
                <Link href="/?tab=champions" style={{ color: '#1ABF9E', textDecoration: 'underline' }}>Return to Champions</Link>
            </div>
        );
    }
