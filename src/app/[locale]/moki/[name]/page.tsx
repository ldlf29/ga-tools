...
const translations: Record<string, { descSuffix: string }> = {
  es: { descSuffix: "Descubre las mejores composiciones y tier list competitiva para aumentar tu win rate." },
  pt: { descSuffix: "Descubra as melhores composições e tier list competitiva para aumentar sua taxa de vitória." },
  vi: { descSuffix: "Khám phá đội hình tốt nhất và danh sách xếp hạng để tăng tỷ lệ thắng của bạn." },
  tl: { descSuffix: "Tuklasin ang pinakamagandang comps at tier list para mapataas ang win rate mo." },
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; name: string }> }): Promise<Metadata> {
...
export default async function LocalizedMokiPage({ params }: { params: Promise<{ locale: string; name: string }> }) {
